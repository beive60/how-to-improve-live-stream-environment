...
- id: AE-02
  title: "VST音声処理チェーン構築 - EQ/コンプレッサー/リミッター/ノイズゲートで配信音声を整える"
  phase: 1
  duration: "20-25分"
  status: in progress
  summary: >
    OBS内蔵フィルタではなくVSTプラグインを使った音声処理チェーンの設計と実装。
    EQ・コンプレッサー・リミッター各プラグインの役割と推奨パラメータを解説したうえで、
    「声が途切れる」「環境音が入る」問題に対処するノイズゲートの閾値設定を追加する。
    スペクトルアナライザで環境音を計測し、適切なゲート閾値を決定する手法をカバーし、
    1本でVSTチェーン全体を完成させる。
...
- id: AE-04
  title: "LUFS音圧最適化 - YouTubeの-14 LUFS基準と正規化の仕組みを理解して音圧不足を根治する"
  phase: 1
  duration: "10-15分"
  status: 未着手
  summary: >
    ...
    モバイル端末での視聴時に音量が小さく聴こえる問題の原因を
    「音圧設計の欠如」として定義し、アルゴリズムに適合した音圧設計の
    考え方を提示することで、配信品質改善への潜在ニーズを顕在化させる
...
- id: AE-10
  title: "Noise Cancelling - AIノイズキャンセリングで配信音声をクリアにする"
  phase: 1
  duration: "15-20分"
  status: 未着手
  summary: >
    NVIDIA BroadcastやRTX Voiceを使用したAIノイズキャンセリングの設定と効果を解説し、
    配信音声のクリア化を実現する方法を紹介する
```
...
- id: VT-02
  title: "OBS色空間・LUT設定の正解 - 601/709、Full/Partialの違いとスマホ視聴時の視認性確保"
  phase: 1
  duration: "15-20分"
  status: 未着手
  summary: >
    ...
    モバイル端末での視聴環境を考慮した視認性確保の観点を追加し、
    「YouTubeのスマホ視聴で色が破綻する」という潜在的な不満に対応する
    フレーミングで構成することで、アルゴリズム適合性を高める
...
強力な差別化コンテンツとして追加。
AE-10（AIノイズキャンセリング）は、AE-03（ノイズゲート）の補完アプローチとして、
検索需要が高く制作容易性も高いためPhase 1に配置。
-->
